...
export default function InputPage() {
  ...
  return <section className="ez-flex ez-flex-row-wrap">
        ...
        <fieldset className="ez-grid ez-fieldset ez-fieldset--grid-2">
          {elementsConfig.map(([type, elementOptions]) => {
            ...
            return <>
              ...
              <div className="ez-input"><input type={type} name={name} id={name} defaultValue={value ?? ''}/></div>
              {isRadioOrCheckbox ? <>
                <label htmlFor={`${name}-checked`}>{humanReadableName} (checked)</label>
                <div className="ez-input"><input type={type} name={`${name}-checked`} id={`${name}-checked`} defaultChecked={true}/></div>
              </> : null}
              {showReadonly || isRadioOrCheckbox ? <>
                <label htmlFor={`${name}-readonly`}>{humanReadableName} (readonly)</label>
                <div className="ez-input"><input type={type} name={`${name}-readonly`} id={`${name}-readonly`}
              ...
              <div className="ez-input"><input type={type} name={`${name}-disabled`} id={`${name}-disabled`}
      ...
      <form action="#">
        <fieldset className="ez-grid ez-fieldset--grid-2">
          ...
          <div className="ez-field">
            <input id="text-input-1" name="text-input-1" className="ez-input"/>
          ...
          <div className="ez-field">
            <input id="text-input-2" name="text-input-2" className="ez-input" disabled/>
          ...
          <div className="ez-field">
            <input id="text-input-5" name="text-input-5" required className="ez-input"/>
          </div>
        </fieldset>
        ...
        <fieldset className={`${styles.fieldset} ez-flex ez-flex-row-wrap`}>
          ...
          {inputTypes.map((inputType) => {
            ...
            return <>
              ...
              <input
                type={inputType}
                className={`${inputType === 'button' ? 'ez-btn ez-primary ez-filled' : ''}`}
